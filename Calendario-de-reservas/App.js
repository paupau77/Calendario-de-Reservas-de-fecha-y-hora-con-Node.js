const monthYearEl = document.getElementById('month-year');
    ...
    let reservedSlots = [];
    let reservedDates = [];
    // URL del backend (ajusta aquí si usas IP LAN)
    const backendURL = 'http://localhost:3000/api/bookings';
    async function fetchReservedSlots() {
      try {
        const resp = await fetch(backendURL);
        reservedSlots = await resp.json();
        // Actualiza reservedDates
        reservedDates = [];
        for (let slot of reservedSlots) {
          const reservasPorDia = reservedSlots.filter(s => s.date === slot.date);
          if (reservasPorDia.length >= availableHours.length && !reservedDates.includes(slot.date)) {
            reservedDates.push(slot.date);
          }
        }
      } catch (e) {
        reservedSlots = [];
